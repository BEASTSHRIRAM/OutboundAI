...
async def approve_draft(id: str, subject: str = None, body: str = None, user: User = Depends(get_current_user)):
    ...
    # 2. Attempt to Send Email via Composio (if connected)
    # 2. Handle Email (if connected and channel is email)
    if draft.channel in ["email", "gmail"] and user.gmail_connection_id:
    ...
    # 3. Handle Voice Call
    elif draft.channel in ["phone", "call"]:
        ...
        if draft.prospect_id:
            prospect = await Prospect.get(draft.prospect_id)
            if prospect:
                recipient_phone = prospect.public_contact
                mission_id = prospect.mission_id
        
        # Fallback to metadata if public_contact is missing
        if not recipient_phone and draft.metadata:
             recipient_phone = draft.metadata.get("recipientPhone")
             
        if recipient_phone:
            try:
                print(f"Approved: Triggering call to {recipient_phone}")
                # Use draft body as the system prompt (intent)
                response = await trigger_call(
                    phone_number=recipient_phone,
                    intent=draft.body,
                    metadata={"mission_id": mission_id, "draft_id": str(draft.id)}
                )
                print(f"Call triggered: {response}")
                
            except Exception as e:
                print(f"Failed to trigger call: {e}")
        else:
            print("Approved: No recipient phone number found for this draft.")
