...
async def trigger_call(
    ...
    if hasattr(settings, 'VAPI_ASSISTANT_ID') and settings.VAPI_ASSISTANT_ID:
        # Simple payload - just use the pre-configured assistant but allow overrides
        payload = {
            ...
            # Allow overriding the pre-configured assistant's settings
            "assistantOverrides": {
        ...
        # Transient assistant with simpler config  
        payload = {
            "phoneNumberId": settings.VAPI_PHONE_NUMBER_ID,
            "customer": {
                "number": phone_number
    ...
    print(f"[VAPI] DEBUG: Using VAPI_PHONE_NUMBER_ID: {settings.VAPI_PHONE_NUMBER_ID}", flush=True)
    print(f"[VAPI] Request payload: {payload}", flush=True)
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            print(f"[VAPI] Response status: {response.status_code}", flush=True)
            print(f"[VAPI] Response body: {response.text}", flush=True)
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"[VAPI STATUS] Call {call_id} current status: {data.get('status')}", flush=True)
                return {
                    "success": True,
                    "call_id": call_id,
                    "status": data.get("status"),
