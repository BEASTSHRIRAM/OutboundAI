...
interface CallResult {
  ...
  status?: string;
  error?: string;
  details?: string;  // Vapi detailed error response
}
  ...
  const handleCall = async () => {
    ...
    try {
      ...
      if (result.success && result.call_id) {
        setCallStatus("Call initiated. Waiting for response...");
  ...
  const pollCallStatus = async (callId: string) => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (status.success) {
          if (status.status === "completed" || (status.status && status.status.startsWith("ended"))) {
      ...
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-400">Make AI-powered phone calls</p>
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 backdrop-blur rounded-2xl p-6 border border-gray-700/50">
          <div className="space-y-6">
            ...
            {/* Result */}
            {callResult && (
              <div className={`p-4 rounded-xl ${callResult.success ? 'bg-green-500/20 border border-green-500/30' : 'bg-red-500/20 border border-red-500/30'}`}>
            ...
            {callStatus && (
              <div className="p-4 bg-blue-500/20 border border-blue-500/30 rounded-xl">
                <p className="text-blue-300">{callStatus}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-6 p-4 bg-gray-800/30 rounded-xl border border-gray-700/30">
          ...
          <ol className="text-sm text-gray-400 space-y-1 list-decimal list-inside">
            ...
            <li>After the call ends, a summary will appear</li>
          </ol>
        </div>
      </div>
    </div>
