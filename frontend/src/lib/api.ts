...
export function useApi() {
    ...
    return useMemo(() => ({
        ...
        // Voice Calls
        initiateCall: async (phoneNumber: string, intent: string, firstMessage?: string) => {
            const res = await fetchWithAuth('/voice/call', {
                method: 'POST',
                body: JSON.stringify({
            ...
            // Always return the JSON response (includes error details from Vapi)
            return res.json();
        },

        getCallStatus: async (callId: string) => {
            const res = await fetchWithAuth(`/voice/call/${callId}`);
